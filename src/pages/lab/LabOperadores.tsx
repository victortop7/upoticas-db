...
const INP: React.CSSProperties = {
  ...
  borderRadius: '8px', color: '#000', outline: 'none',
...
export default function LabOperadores() {
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '700px' }}>
      ...
      {sucesso && (
        <div style={{ background: 'rgba(0,102,0,0.15)', border: '1px solid #006600', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px', fontSize: '13px', color: '#006600' }}>
          {sucesso}
        </div>
      )}

      {showForm && (
        <form onSubmit={handleSalvar} style={{ background: '#d4d0c8', border: '1px solid #b0aca4', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
          <div style={{ fontSize: '13px', fontWeight: '700', color: '#000', marginBottom: '16px' }}>Novo Operador</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div>
              <label style={{ fontSize: '11px', fontWeight: '600', color: '#666', display: 'block', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Nome *</label>
              <input value={nome} onChange={e => setNome(e.target.value)} required style={INP} placeholder="Nome completo" />
            </div>
            <div>
              <label style={{ fontSize: '11px', fontWeight: '600', color: '#666', display: 'block', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>E-mail *</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required style={INP} placeholder="email@exemplo.com" />
            </div>
          </div>
          <div style={{ marginBottom: '16px' }}>
            <label style={{ fontSize: '11px', fontWeight: '600', color: '#666', display: 'block', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Senha *</label>
      ...
      {loading ? (
        <div style={{ color: '#666', fontSize: '13px' }}>Carregando...</div>
      ) : (
        <div style={{ background: '#d4d0c8', border: '1px solid #b0aca4', borderRadius: '12px', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {operadores.length === 0 ? (
                <tr>
                  <td colSpan={3} style={{ padding: '24px 16px', textAlign: 'center', fontSize: '13px', color: '#666' }}>
                    Nenhum operador cadastrado
                  </td>
                </tr>
              ) : operadores.map((op, i) => (
                <tr key={op.id} style={{ borderBottom: i < operadores.length - 1 ? '1px solid #b0aca4' : 'none' }}>
                  <td style={{ padding: '12px 16px', fontSize: '13px', color: '#000', fontWeight: '600' }}>{op.nome}</td>
                  <td style={{ padding: '12px 16px', fontSize: '13px', color: '#555', fontFamily: "'Courier New', monospace" }}>{op.email}</td>
