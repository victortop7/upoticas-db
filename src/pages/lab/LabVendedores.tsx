...
const INP: React.CSSProperties = { width: '100%', padding: '7px 10px', fontSize: '13px', boxSizing: 'border-box', background: 'var(--surface-alt)', border: '1px solid var(--border)', borderRadius: '7px', color: 'var(--text)', outline: 'none', fontFamily: 'var(--mono)' };
...
const CARD: React.CSSProperties = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px', padding: '16px' };
  ...
  if (modo !== 'lista') return (
    <div style={{ padding: '24px', maxWidth: '820px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
        ...
        <h1 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: 'var(--text)' }}>{modo === 'novo' ? 'Novo Vendedor/Operador' : `Editar — ${sel?.nome}`}</h1>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div style={CARD}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 80px', gap: '10px' }}>
            ...
            <div><label style={LBL}>UF</label><select value={form.estado} onChange={e => set('estado', e.target.value)} style={{ ...INP, fontFamily: 'var(--sans)' }}><option value="">—</option>{ESTADOS_BR.map(uf => <option key={uf} value={uf}>{uf}</option>)}</select></div>
          </div>
        </div>
        <div style={CARD}>
          ...
          <div><label style={LBL}>Observações</label><textarea value={form.observacoes} onChange={e => set('observacoes', e.target.value)} rows={2} style={{ ...INP, fontFamily: 'var(--sans)', resize: 'vertical' }} /></div>
        </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px' }}>
        {loading ? <div style={{ padding: '48px', textAlign: 'center', color: 'var(--text-muted)' }}>Carregando...</div>
          ...
          : <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {filtrados.map(v => (
                  <tr key={v.id} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }} onClick={() => openEditar(v)}
                    ...
                    <td style={{ padding: '10px 12px', fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--text-dim)' }}>{String(v.codigo).padStart(2, '0')}</td>
                    <td style={{ padding: '10px 12px', fontSize: '13px', fontWeight: '600', color: 'var(--text)' }}>{v.nome}</td>
                    <td style={{ padding: '10px 12px', fontSize: '12px', fontFamily: 'var(--mono)', color: 'var(--text-dim)' }}>{v.cpf_cnpj ?? '—'}</td>
                    <td style={{ padding: '10px 12px', fontSize: '12px', fontFamily: 'var(--mono)', color: 'var(--text-dim)' }}>{v.pct_comissao != null ? `${v.pct_comissao}%` : '—'}</td>
                    <td style={{ padding: '10px 12px', fontSize: '12px', fontFamily: 'var(--mono)', color: 'var(--text-dim)' }}>{v.telefone ?? '—'}</td>
                    <td style={{ padding: '10px 12px' }}><button onClick={e => { e.stopPropagation(); excluir(v.id); }} style={{ fontSize: '12px', color: 'var(--red)', background: 'none', border: 'none', cursor: 'pointer' }}>✕</button></td>
                  </tr>
                ))}
              </tbody>
            </table>}
