...
const INP: React.CSSProperties = { width: '100%', padding: '7px 10px', fontSize: '13px', boxSizing: 'border-box', background: '#dedad2', border: '1px solid #b0aca4', borderRadius: '7px', color: '#000', outline: 'none', fontFamily: "'Courier New', monospace" };
...
const CARD: React.CSSProperties = { background: '#d4d0c8', border: '1px solid #b0aca4', borderRadius: '10px', padding: '16px' };
  ...
  if (modo !== 'lista') return (
    <div style={{ padding: '24px', maxWidth: '820px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
        ...
        <h1 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#000' }}>{modo === 'novo' ? 'Novo Vendedor/Operador' : `Editar — ${sel?.nome}`}</h1>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
          ...
          <button onClick={() => setModo('lista')} style={{ padding: '9px 22px', fontSize: '13px', background: 'transparent', color: '#555', border: '1px solid #b0aca4', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit' }}>Cancelar</button>
  ...
  return (
    <div style={{ padding: '28px', maxWidth: '860px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#000' }}>Vendedores / Operadores</h1>
      ...
      <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar..." style={{ ...INP, marginBottom: '16px', background: '#d4d0c8', width: '300px' }} />
      <div style={{ background: '#d4d0c8', border: '1px solid #b0aca4', borderRadius: '10px' }}>
        {loading ? <div style={{ padding: '48px', textAlign: 'center', color: '#666' }}>Carregando...</div>
          : filtrados.length === 0 ? <div style={{ padding: '48px', textAlign: 'center', color: '#666' }}>Nenhum vendedor. <button onClick={openNovo} style={{ color: '#005500', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '600' }}>Cadastrar →</button></div>
          : <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr style={{ background: '#dedad2', borderBottom: '1px solid #b0aca4' }}>
                {['Cód', 'Nome', 'CPF/CNPJ', '% Comissão', 'Telefone', ''].map(h => <th key={h} style={{ padding: '9px 12px', textAlign: 'left', fontSize: '10px', fontWeight: '600', color: '#666', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>)}
              </tr></thead>
              <tbody>
                {filtrados.map(v => (
                  <tr key={v.id} style={{ borderBottom: '1px solid #b0aca4', cursor: 'pointer' }} onClick={() => openEditar(v)}
                    onMouseEnter={e => (e.currentTarget.style.background = '#dedad2')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <td style={{ padding: '10px 12px', fontFamily: "'Courier New', monospace", fontSize: '12px', color: '#555' }}>{String(v.codigo).padStart(2, '0')}</td>
                    <td style={{ padding: '10px 12px', fontSize: '13px', fontWeight: '600', color: '#000' }}>{v.nome}</td>
                    <td style={{ padding: '10px 12px', fontSize: '12px', fontFamily: "'Courier New', monospace", color: '#555' }}>{v.cpf_cnpj ?? '—'}</td>
                    <td style={{ padding: '10px 12px', fontSize: '12px', fontFamily: "'Courier New', monospace", color: '#555' }}>{v.pct_comissao != null ? `${v.pct_comissao}%` : '—'}</td>
                    <td style={{ padding: '10px 12px', fontSize: '12px', fontFamily: "'Courier New', monospace", color: '#555' }}>{v.telefone ?? '—'}</td>
