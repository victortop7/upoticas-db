    ...
    return (
      <div>
        ...
        <div style={{ marginTop: '14px', border: '1px solid #a0a098' }}>
          <div style={{ background: '#000080', color: '#fff', fontSize: '10px', fontWeight: 'bold', padding: '2px 6px', letterSpacing: '1px' }}>IMPORTAÇÃO PEDIDO ON-LINE</div>
        ...
        <div style={{ marginTop: '14px', border: '1px solid #a0a098' }}>
          <div style={{ background: '#000080', color: '#fff', fontSize: '10px', fontWeight: 'bold', padding: '2px 6px', letterSpacing: '1px' }}>OBSERVAÇÕES EM PEDIDOS</div>
          <div style={{ padding: '8px 10px', background: '#d4d0c8' }}>
...
function TabelaContent({ tabela, config, onChange }: {
  ...
  const th: React.CSSProperties = {
    background: '#000080', color: '#fff', padding: '4px 6px', fontSize: '11px', fontWeight: 'bold',
  };
          ...
          <div style={S.panelBody()}>
            {Array.from({ length: 10 }, (_, i) => (
              <div
                ...
                onMouseEnter={e => { if (despGrupo !== i) (e.currentTarget as HTMLElement).style.background = '#a0a4c8'; }}
      ...
      <div>
        ...
        <div style={{ padding: '6px 8px', display: 'flex', gap: '4px', justifyContent: 'center', background: '#d4d0c8', border: '2px inset #808080', borderTop: 'none' }}>
          {[0,1,2,3,4].map(pg => {
            ...
            return (
              <button key={pg} onClick={() => setBancoPg(pg)} style={{ padding: '3px 10px', fontSize: '11px', fontWeight: 'bold', fontFamily: 'inherit', background: bancoPg === pg ? '#880000' : '#d4d0c8', color: bancoPg === pg ? '#fff' : '#000080', border: bancoPg === pg ? '2px inset #808080' : '2px outset #808080', cursor: 'pointer' }}>
        ...
        <div style={{ ...S.panelBody(), padding: '10px 12px' }}>
          {codes.map(code => (
            <div key={code} style={{ display: 'flex', alignItems: 'center', marginBottom: '5px', gap: '6px' }}>
              <span style={{ fontSize: '11px', fontWeight: 'bold', color: '#000080', minWidth: '56px' }}>LISTA {code}</span>
  ...
  const SaveBar = () => (
    <div style={{ marginTop: '12px', display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
      <button
        onClick={handleSave}
        disabled={saving}
        style={{ padding: '3px 18px', fontSize: '11px', fontWeight: 'bold', background: saving ? '#808080' : '#000080', color: '#ffffff', border: '1px outset #8080ff', cursor: saving ? 'default' : 'pointer', fontFamily: 'inherit', letterSpacing: '0.5px' }}
      >
    ...
    return items.map((t, i) => {
      ...
      return (
        <div
          ...
          onMouseEnter={e => { if (!act) (e.currentTarget as HTMLElement).style.background = '#a0a4c8'; }}
