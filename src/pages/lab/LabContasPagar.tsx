  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <div style={{ padding: '14px 20px', borderBottom: '1px solid #b0aca4', background: '#d4d0c8', display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
        <h2 style={{ margin: 0, fontSize: '17px', fontWeight: '700', color: '#000', marginRight: '8px' }}>Contas a Pagar</h2>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {loading ? <div style={{ padding: '60px', textAlign: 'center', color: '#666' }}>Carregando...</div>
          : filtradas.length === 0 ? <div style={{ padding: '60px', textAlign: 'center', color: '#666' }}>Nenhum lançamento.</div>
              ...
              <tbody>
                {filtradas.map(c => (
                  <tr key={c.id} style={{ borderBottom: '1px solid #b0aca4' }}
                    ...
                    <td style={{ padding: '9px 12px', fontSize: '12px', color: '#000', maxWidth: '140px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.fornecedor || '—'}</td>
                    <td style={{ padding: '9px 12px', fontSize: '12px', color: '#555', maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.descricao}</td>
                    <td style={{ padding: '9px 12px', fontSize: '11px', color: '#555' }}>{c.categoria ?? '—'}</td>
      ...
      {baixando && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#d4d0c8', border: '1px solid #b0aca4', borderRadius: '12px', padding: '24px', width: '380px' }}>
            <h3 style={{ margin: '0 0 12px', fontSize: '15px', fontWeight: '700', color: '#000' }}>Pagar — #{String(baixando.numero).padStart(4,'0')}</h3>
            <div style={{ fontSize: '13px', color: '#555', marginBottom: '14px' }}>{baixando.descricao} — <b style={{ color: '#000' }}>{brl(baixando.valor)}</b></div>
      ...
      {novaModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#d4d0c8', border: '1px solid #b0aca4', borderRadius: '12px', padding: '24px', width: '440px' }}>
            <h3 style={{ margin: '0 0 16px', fontSize: '15px', fontWeight: '700', color: '#000' }}>Novo Lançamento — Contas a Pagar</h3>
