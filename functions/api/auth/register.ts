...
export const onRequestPost = async ({ request, env }: { request: Request; env: Env }) => {
  try {
    const body = await request.json() as { nome_otica: string; nome: string; email: string; senha: string; tipo?: string };
    ...
    const tipo = body.tipo === 'lab' ? 'lab' : 'otica';
    ...
    await env.DB.batch([
      env.DB.prepare('INSERT INTO tenants (id, nome, email, tipo, plano, trial_expira) VALUES (?, ?, ?, ?, ?, ?)')
        .bind(tenantId, body.nome_otica, body.email, tipo, 'trial', trialExpira),
    ...
    const tenant = { id: tenantId, nome: body.nome_otica, email: body.email, tipo, plano: 'trial', trial_expira: trialExpira, ativo: true };
